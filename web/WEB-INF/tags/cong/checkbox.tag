...
<c:catch var="exception">
    ...
    <c:choose>
        <c:when test="${not empty form}">
            <html:checkbox property="${name}" title="${title}" name="${form}" alt="${alt}" styleId="${id}" styleClass="${styleClass}" disabled="${disabled}" onclick="${onclick}" onchange="${onchange}"/>
        ...
        <c:otherwise>
            ...
            <input type="checkbox" name="${name}" id="${id}" alt="${alt}" title="${title}" class="${styleClass}" ${disabled}  onclick="${onclick}" onchange="${onchange}" ${checked}/>
