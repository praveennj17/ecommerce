...
<%@attribute name="publicAccess" type="java.lang.Boolean"%>
...
<%
            request.setAttribute("currentTime", new java.util.Date().getTime());
%>
...
<c:if test="${publicAccess}">
    <input type="hidden" id="publicAccess" value="${publicAccess}"/>
</c:if>
