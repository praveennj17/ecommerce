 ...
 * @author Sunil
 */
public class BaseHibernateDAO<T extends Domain> extends IBaseHibernateDAO implements Serializable {
    ...
    /**
     * This constructor is must for Domain class which requires to search
     * result, execute query etc.
     *
...
//            getCurrentSession().flush();
    ...
    /**
     * Synchronize the DB and instance Domain. Update should be used to save the
     * data when the session does not contain an already persistent instance
...
//            getCurrentSession().flush();
    ...
    /**
     * Synchronize the DB and instance Domain. Merge should be used to save the
     * modifications at any time without knowing about the state of a session.
     *
...
//            getCurrentSession().flush();
    ...
    /**
     * Flush and Clear the session If any changes made to session. It will save
     * all unsaved data.
     */
    ...
    /**
     * Get the List of Domain by matching a property of many value.
     *
     * @param propertyName - Property of Domain <T>
     * @param value - value
     * @return - List of Type Domain
    ...
    /**
     * Find the result from the list of properties which contains the propery
     * name and value.
     *
     * @param Properties
     * @param lo
     ...
     * @param Properties
     * @param lo - Logical Operator AND or OR
     * @param appendAtEnd any valid syntax used at the end of query like 'ORDER
     * BY ', 'GROUP BY', 'LIMIT 10' etc.
     * @return
     */
...
//        System.out.println("queryObject : " + queryObject.getQueryString());
    ...
    private StringBuilder generateCondition(List<Property> Properties, LogicalOperator lo) {
        ...
        List<Property> removeList = new ArrayList<>();
     ...
     * @return - List of Type Domain
     * @throws NoSuchMethodException - If the property mentioned the list <code> properties
     * </code> does not match with the property of instance
     * @throws IllegalAccessException - if the property of instance is private.
     * @throws IllegalArgumentException - if the argument type doesn't not
     * match.
    ...
    public void executeSQLUpdate(String queryString) {
        log.debug("query : " + queryString);
        try {
            SQLQuery query = getCurrentSession().createSQLQuery(queryString);
            query.executeUpdate();
    ...
    public void executeSQLUpdateWithParameter(String queryString, String parameterName, String parameterValue) {
        log.debug("query : " + queryString);
        try {
            SQLQuery query = getCurrentSession().createSQLQuery(queryString);
            query.setParameter(parameterName, parameterValue);
    ...
    public T getByProperties(List<Property> properties) {
    ...
    public List<T> findByProperties(List<Property> properties) {
        Criteria criteria = createCriteria(clazz);
        for (Property property : properties) {
            criteria.add(Restrictions.eq(property.getName(), property.getValue()));
        }
    ...
    public void evict(Domain domain) {
        if (domain.getId() != null) {
            ...
            log.debug("evict " + clazz + " instance");
        }
    }
    ...
    public Long getRowCount() {
        ...
        SQLQuery query = getCurrentSession().createSQLQuery("select count(*) as rowCount from " + tableName);
        query.addScalar("rowCount", LongType.INSTANCE);
